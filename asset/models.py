...
class Asset(models.Model):
    ...
    manufacturer = models.ForeignKey('Manufacturer', null=True, blank=True, on_delete=models.CASCADE, verbose_name='制造商')
    ...
    contract = models.ForeignKey('Contract', null=True, blank=True, on_delete=models.CASCADE, verbose_name='合同')
    ...
    approved_by = models.ForeignKey(User, null=True, blank=True, verbose_name='批准人', on_delete=models.CASCADE, related_name='approved_by')
...
class NIC(models.Model):
    ...
    def __str__(self):
        return '%s:  %s:  %s:   %s' % (self.asset.name, self.model, self.mac, self.ip_address)
...
class EventLog(models.Model):
    ...
    asset = models.ForeignKey('Asset', blank=True, null=True, on_delete=models.CASCADE)  # 当资产审批成功时有这项数据
    new_asset = models.ForeignKey('NewAssetApprovalZone', blank=True, null=True, on_delete=models.CASCADE)  # 当资产审批失败时有这项数据
    ...
    user = models.ForeignKey(User, blank=True, null=True, verbose_name='事件执行人', on_delete=models.CASCADE)  # 自动更新资产数据时没有执行人
    memo = models.TextField('备注', blank=True, null=True)
